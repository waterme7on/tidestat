...
const tidestatEvents = { page_viewed: 'page_view', product_viewed: 'custom', product_added_to_cart: 'custom', checkout_started: 'checkout', checkout_completed: 'purchase' };
for (const [shopifyEvent, type] of Object.entries(tidestatEvents)) {
  analytics.subscribe(shopifyEvent, event => {
    tidestatQueue = tidestatQueue.then(async () => {
      ...
      const location = new URL(event.context.window.location.href);
      ...
      if (!state.session_id || !Number.isFinite(state.last_seen) || occurredAt - state.last_seen >= 1800000) {
        let source = 'direct';
        try { const referrer = new URL(event.context.document.referrer); if (referrer.hostname !== location.hostname) source = referrer.hostname; } catch { /* Direct. */ }
        state.session_id = `shopify-${event.id}`;
        state.acquisition = { source: location.searchParams.get('utm_source') || source, medium: location.searchParams.get('utm_medium') || (source === 'direct' ? 'none' : /google\.|bing\.|duckduckgo\./i.test(source) ? 'organic' : 'referral'), campaign: location.searchParams.get('utm_campaign') || '' };
      }
      ...
      await fetch(TIDESTAT_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json' }, keepalive: true, credentials: 'omit', body: JSON.stringify({
        ...
        path: location.pathname, referrer: '', occurred_at: occurredAt, acquisition: state.acquisition,
        properties: { ...(type === 'custom' ? { name: shopifyEvent } : {}), ...(checkout?.order?.id ? { order_id: checkout.order.id } : {}) }
      }) });
