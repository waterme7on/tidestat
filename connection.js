...
(() => {
 ...
 const api={
  ...
  set(site,token){if(token){save({site,token});window.dispatchEvent(new Event('tide:sitechange'));}else this.select(site);},
  ...
  clear(){epoch++;account={};try{sessionStorage.removeItem(key);}catch{}window.dispatchEvent(new Event('tide:sitechange'));},
  async refresh(){const before=JSON.stringify(this.get());const ticket=++epoch;try{
  ...
  }catch{if(ticket===epoch)account={};}if(ticket===epoch&&before!==JSON.stringify(this.get()))window.dispatchEvent(new Event('tide:sitechange'));return this.get();},
