...
(() => {
 ...
 const api={
  ...
  select(site){if(!account.sites?.some(s=>s.id===site))return false;save({site});const url=new URL(location.href);if(url.searchParams.has('site')){url.searchParams.set('site',site);history.replaceState(null,'',url);}window.dispatchEvent(new Event('tide:sitechange'));return true;},
  ...
  async refresh(){const ticket=++epoch;try{
   ...
   if(ticket!==epoch)return this.get();account={...result,sites};const saved=read(),requested=new URLSearchParams(location.search).get('site');if(!saved.token&&sites.some(s=>s.id===requested)){save({site:requested});return this.get();}if(!saved.token&&(!saved.site||!sites.some(s=>s.id===saved.site)))save(sites.length?{site:sites[0].id}:{});
