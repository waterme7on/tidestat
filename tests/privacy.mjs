...
const bindings=[];
const DB={prepare(sql){return {bind(...args){bindings.push(args);return {first:async()=>db.prepare(sql).get(...args)||null,run:async()=>db.prepare(sql).run(...args),all:async()=>({results:db.prepare(sql).all(...args)})}}}},async batch(statements){db.exec('BEGIN');try{const results=await Promise.all(statements.map(s=>s.run()));db.exec('COMMIT');return results;}catch(e){db.exec('ROLLBACK');throw e;}}};
const env={DB,SITES_JSON:JSON.stringify({test:{origin:'https://example.test',readToken:'test-token'}}),ASSETS:{fetch:()=>new Response('asset')}};
async function collect(id,ip,path='/zh/work',extra={}){const req=new Request('https://example.test/api/collect',{method:'POST',headers:{'content-type':'application/json',origin:'https://example.test',...(ip?{'CF-Connecting-IP':ip}:{})},body:JSON.stringify({schema_version:1,event_id:crypto.randomUUID(),site_id:'test',visitor_id:id,session_id:'session-'+id,type:'page_view',occurred_at:Date.now(),path,ip:'192.0.2.99',...extra})});Object.defineProperty(req,'cf',{value:{city:'Tokyo',country:'JP',latitude:'35.7',longitude:'139.7'}});return worker.fetch(req,env);}
...
const body=await (await worker.fetch(new Request('https://example.test/api/live?site=test',{headers:{authorization:'Bearer test-token'}}),env)).json();
assert.equal(body.onlineMs,90000);assert.equal(body.visitors.length,4);
assert.equal(body.visitors.find(v=>v.id==='visitor-one').maskedIp,'203.*.*.42');
assert.equal(body.visitors.some(v=>v.id==='legacy'),false);assert.equal(body.visitors.find(v=>v.id==='unknown').maskedIp,null);
