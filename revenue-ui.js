...
function render() {
  ...
  if(current==='journeys') $('content').innerHTML=`<section class="card"><div class="card-head"><div><h2>Revenue by journey</h2><p class="muted">Observed paths before payment · repeated sessions remain part of the story</p></div>${currencySelect}</div>${ranking(data.journeys,'Journey')}</section>${funnelHTML()}${data.signupFunnel?funnelHTML(data.signupFunnel,'Signup funnel'):''}`;
...
function ranking(rows=[], label, perVisitor=false) {
  ...
  return `<div class="table-scroll"><table><thead><tr><th>${label}</th><th>Payments*</th>${perVisitor?'<th>Visitors</th><th>Revenue / visitor</th>':''}<th>${esc(currency)} revenue</th></tr></thead><tbody>${sorted.map(r=>`<tr><td>${esc(r.name)}<div class="bar"><b style="width:${Math.max(0,(r.revenue?.[currency]||0)/max*100)}%"></b></div></td><td>${number(r.payments)}</td>${perVisitor?`<td>${number(r.visitors)}</td><td>${r.visitors?money((r.revenue?.[currency]||0)/r.visitors):'—'}</td>`:''}<td>${money(r.revenue?.[currency]||0)}</td></tr>`).join('')}</tbody></table></div><p class="muted">* Payment counts span currencies. Revenue is shown only in ${esc(currency)}.</p>`;
}
function funnelHTML(rows=data.funnel||[],title='Revenue funnel') {const max=Math.max(1,...rows.map(r=>r.visitors));return `<section class="card"><h2>${title}</h2><p class="muted">Recorded visitor milestones · ${number(data.overview.visitors)} visitors in the selected period</p>${rows.map(r=>`<div class="funnel-row"><span>${esc(r.name)}</span><div class="bar"><b style="width:${100*r.visitors/max}%"></b></div><strong>${number(r.visitors)}</strong></div>`).join('')}<p class="muted">Payments may come from returning visitors with steps recorded before this period.</p></section>`;}
